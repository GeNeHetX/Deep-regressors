...
def train_model(model: nn.Module,
    ...
    train_iou_metric = BinaryJaccardIndex().to(device)
    val_iou_metric = BinaryJaccardIndex().to(device)
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Progress", unit="epoch"):
        ...
        history['train_iou'].append(train_iou_metric.compute().item())
        ...
        history['val_iou'].append(val_iou_metric.compute().item())
        ...
        tqdm.write(f"Epoch [{epoch+1}/{num_epochs}], Train Loss: {avg_train_loss:.2e}, Val Loss: {avg_val_loss:.2e}, Train IOU: {history['train_iou'][-1]:.4f}, Val IOU: {history['val_iou'][-1]:.4f}")
