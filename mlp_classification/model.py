...
def mlp_architecture(input_dim: int, output_dim: int=1, hidden_dim: int=256, num_hidden_layers: int=2, architecture_factor: float = 1, dropout: float=0.0, classification: bool=False) -> list:
    """
    ...
    Args:
        ...
        classification (bool): Whether the task is classification (adds activation at the end).
    Returns:
...
class MLPClassifier(nn.Module):
    """
    Flexible Multi-Layer Perceptron (MLP) for classification using built-in PyTorch modules.
    """
    def __init__(self, input_dim: int, output_dim: int=2, hidden_dim: int=256, num_hidden_layers: int=2, architecture_factor: float = 1, dropout: float=0.0):
        """
        Args:
            input_dim (int): Number of input features.
        ...
        layers = mlp_architecture(input_dim, output_dim, hidden_dim, num_hidden_layers, architecture_factor, dropout, classification=True)
        self.mlp = nn.Sequential(*layers)
